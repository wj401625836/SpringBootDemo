                            ...
                            <#list statusList as item >
                                <#if item.status == 0>
                                    <li class="warning-element">
                                        <input type="hidden" name="status1Ids" value="${item.id }"/>
                                         ${item.name }
                                    </li>
                                </#if>
                            </#list>
                            ...
                            <#list statusList as item >
                                <#if item.status == 1>
                                    <li class="success-element">
                                        <input type="hidden" name="status2Ids" value="${item.id }"/>
                                        ${item.name }
                                    </li>
                                </#if>
                            </#list>
                    ...
                    <div class="ibox-content">
                        ...
                        <ul class="sortable-list connectList agile-list">
                            <#list statusList as item >
                                <#if item.status == 2>
                                    <li class="warning-element">
                                        <input type="hidden" name="status3Ids" value="${item.id }"/>
                                        ${item.name }
                                    </li>
                                </#if>
                            </#list>
            ...
            $("#frm").validate({
                ...
                submitHandler:function(form){
                    $.ajax({
                        type: "POST",
                        dataType: "json",
                        url: "${ctx!}/plan/status/changeSorted",
                        data: $(form).serialize(),
